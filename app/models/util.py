...
def _get_user_by_token(conn, token: str) -> schemas.SafeUser | None:
    res = conn.execute(
...
def _get_room_from_room_id(conn, room_id: int) -> schemas.SafeRoom | None:
    res = conn.execute(
        text(
...
def _change_room_owner(
    conn, room_id: int, owner: schemas.SafeUser, users_in_room: list
) -> None:
